<template>
  <div class="organization">
    ...
    <div class="height__body">
      ...
      >
        <div class="organization__item">{{ cat.name }}</div>
      </GSwiper>

      <GModal :open="openModal" @close="closeModal()">
        <h2 v-if="!isEdit">Добавить организацию</h2>
        <h2 v-else>Изменить организацию</h2>

        <VInput v-model="category">Название организации</VInput>
...
const categories = computed(() => useOrganizationsStore().organizations);
...
async function addCategory(payload) {
  await useOrganizationsStore().postOrganization(payload);
  closeModal();
}
async function editCategory() {
  await useOrganizationsStore().updateOrganization(
    category.value,
    categoryId.value
  );
  closeModal();
...
async function deleteСategory(id) {
  await useOrganizationsStore().deleteOrganization(id);
}
</script>
